...
### `StompAuthChannelInterceptorTest` — `unit` — аутентификация, аллоулист SEND и deny-by-default на SUBSCRIBE (beads g9x, bwh)
...
- **`everyFrontendSubscriptionFromAuditPasses`** (beads bwh) — `SUBSCRIBE` на все 7 адресов из аудита фронта: `/mutual/chatlist/change_chatpreview/9`, `/mutual/chatlist/list_update/9`, `/mutual/chatlist/image/9`, `/mutual/chatlist/typing/9` (chatlist.view.js) и `/mutual/chat/5`, `/mutual/typing/5`, `/mutual/chat_image/5` (chat.view.js, пользователь участник чата 5) → все проходят. Зачем: **acceptance 4** тикета и единственная защита от главного риска deny-by-default — молча отрезать живую подписку. Список синхронизирован с белым списком в `StompAuthChannelInterceptor`; новая подписка во фронте обязана появиться и здесь, и там.
