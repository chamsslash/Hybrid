...
public class HttpService {
    ...
    public ResponseEntity<?> BindFP(@RequestHeader(value ="X-Fingerprint")String fingerprint,
        ...
        log.info("ПОЛУЧЕНА СЫРАЯ СТРОКА FPCOMPONENTS >>>{}<<<", fpparts);        HttpServletRequestWrapper fakeRequest = new HttpServletRequestWrapper(request) {
...
//        String state = stateResolver.bindFingerPrint(stateResolver.hash(request.getHeader("X-Fingerprint"),fpparts));
        String ptr=  new ReverseDnsResolver().getPTR(json.get("ip").getAsString());
...
//        String resultJson = gson.toJson(Parts);
        String resultJson;
        try{resultJson = stateResolver.JsonStreamingParsing(fpparts);}catch (Exception e){
        ...
        OAuth2AuthorizationRequest oAuth2AuthorizationRequest =  reqresolver.resolve(
                fakeRequest,"google");
        OAuth2AuthorizationRequest customrequest =reqresolver.createAuthorizationRequest(request,oAuth2AuthorizationRequest,state);
        authrepo.saveAuthorizationRequest(customrequest,request,response);
        return  ResponseEntity.status(HttpStatus.OK).body(Map.of("redirectUrl",customrequest.getAuthorizationRequestUri()));
