...
# пользователь не в группе docker. И то и другое всплыло бы только на первом `docker
# build` — то есть поздно и не там, ровно как было с отсутствующим kind.
if ! docker info >/dev/null 2>&1; then
  echo "✗ docker установлен, но демон недоступен" >&2
  echo "  проверь, что он запущен:  systemctl status docker" >&2
  echo "  и что ты в группе docker: sudo usermod -aG docker \$USER" >&2
  echo "  членство подхватывается при ВХОДЕ — после usermod нужно перелогиниться" >&2
  echo "  (newgrp docker действует только в текущей оболочке)" >&2
  exit 1
fi

# Свободное место там, где docker реально хранит данные. Предупреждение, а не отказ:
# точную потребность не предсказать, а останавливать раскатку по догадке хуже, чем дать
# человеку решить самому. Порог грубый — три образа с JRE плюс нода кластера с etcd.
#
# Смотрим именно DockerRootDir, а не /mnt/data: если перенос data-root не сделан или
# монтирование не приехало, место кончится на загрузочном диске, и предупреждение должно
# прийти про него.
docker_root="$(docker info --format '{{.DockerRootDir}}' 2>/dev/null || true)"
if [ -n "$docker_root" ] && [ -d "$docker_root" ]; then
  free_gb="$(df -Pk "$docker_root" | awk 'NR==2 {print int($4/1048576)}')"
  echo "▶ docker хранит данные в ${docker_root} (свободно ~${free_gb} ГБ)"
  if [ "${free_gb:-0}" -lt 10 ]; then
    echo "⚠ меньше 10 ГБ свободно — сборка трёх образов и нода кластера могут не поместиться." >&2
    echo "  Если данные должны лежать на отдельном диске, сверься с docs/vm-data-disk.md:" >&2
    echo "  путь выше обязан указывать на него, а не на загрузочный раздел." >&2
  fi
fi
...
# В ПУБЛИЧНОМ режиме дефолты другие, и это не удобство, а исправление грабель. Перед
# кластером стоит прокси, который обязан держать 80 и 443 (порты ACME не выбираются),
# значит кластеру они не просто не нужны — он с прокси за них дерётся. Поймано живьём:
# при переносе только HTTP-порта kind упал на `failed to bind host port 127.0.0.1:443:
# address already in use`, потому что HTTPS-маппинг остался на 443. Забыть ОДНУ из двух
# переменных слишком легко, поэтому режим задаёт обе разом.
#
# 443 кластеру не нужен и по существу: TLS терминирует прокси, внутрь трафик идёт по
# обычному HTTP. Маппинг оставлен только чтобы не плодить ветку в шаблоне конфига kind.
#
# Локальные дефолты не тронуты: 80/443 на всех интерфейсах, как было.
if [ "$PUBLIC_MODE" = "true" ]; then
  : "${KIND_HTTP_HOST_PORT:=8081}"
  : "${KIND_HTTPS_HOST_PORT:=8443}"
...
#
# В публичном режиме включается по умолчанию: там прокси перед кластером есть всегда, а
# без флага схема теряется молча — приложение работает, но собирает http-адреса на
# рабочем https, и искать причину идёшь в приложение.
if [ "$PUBLIC_MODE" = "true" ]; then
...
# Доверие к X-Forwarded-* имеет смысл ровно до тех пор, пока прислать их может только
# доверенный прокси. Если ingress слушает не loopback, эти заголовки может подделать кто
# угодно снаружи — и клиент сам объявит схему своего соединения. Комбинация не
# запрещается (бывает внешний балансировщик перед машиной), но названа вслух: молча
# оставленный 0.0.0.0 здесь и есть дыра.
case "$KIND_LISTEN_ADDRESS" in
  127.0.0.1|localhost|::1) ;;
  *)
    if [ "$INGRESS_USE_FORWARDED_HEADERS" = "true" ]; then
      echo "⚠ INGRESS_USE_FORWARDED_HEADERS=true при KIND_LISTEN_ADDRESS=${KIND_LISTEN_ADDRESS}." >&2
      echo "  ingress доступен не только с loopback, значит X-Forwarded-Proto может прислать" >&2
      echo "  кто угодно, кто до него дотянется, и объявить своё соединение защищённым." >&2
      echo "  Безопасно только если доступ к этому адресу закрыт снаружи." >&2
    fi
    ;;
esac
