...
function requestAIResponse() {
    ...
    // Подсказкой считается ТОЛЬКО успешный ответ (beads n2f). Раньше сервер отдавал
    // неудачу генерации как 200 с текстом ошибки в теле, и она попадала сюда же —
    // в панель с кнопкой «Вставить». Пользователю предлагали отправить в чат строку
    // «Извините, сервис временно недоступен…», а отправленная, она ещё и уезжала в
    // контекст ассистента наравне с настоящими репликами. Теперь у неудачи свой статус
    // (422 — нечего генерировать, 503 — сорвалось), и она уходит в .catch ниже.
    api.post('/AiAssist', formBody)
        ...
        .catch(err => {
            ...
            // Тело ответа — text/plain и написано для человека («в этом чате пока нет
            // сообщений…»), поэтому показываем его, а не общее «ошибка». Показываем
            // ТОСТОМ: тост нельзя вставить в поле ввода, в отличие от панели подсказки.
            // Строкой сообщение бывает не всегда: сетевой сбой приходит без response.
            const serverText = typeof err?.response?.data === 'string'
                ? err.response.data.trim()
                : '';
            showToast(serverText || 'Ошибка при запросе к AI', 'error');
        });
}
