 ...
 * ОТПРАВИТЕЛЯ — поток WebSocket-контейнера ({@code http-nio-*}), а не пул
 * {@code clientInboundChannel}: {@code AbstractMessageChannel.send} выполняет
 * {@code applyPreSend} синхронно на вызывающем потоке и только потом отдаёт сообщение
 * executor'у канала (тот же вывод, что и в javadoc {@code StompFrameTimestampInterceptor}
 * про «поток отправителя»). Разница не только в имени: у пула {@code http-nio-*} ёмкость —
 * сотни потоков Tomcat, у {@code clientInboundChannel} — дефолт Spring, примерно два на
 * ядро. Практический вывод не меняется: блокировать на этом пути всё равно плохая идея —
 * заминка MessegerParody без единой ошибки в логах вешает весь WebSocket-ярус. Проверка
 * перенесена в контроллер, где список участников уже под рукой.
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        switch (accessor.getCommand()) {
            ...
            case SEND -> {
                ...
                // gRPC-вызов и не держать на .block() поток отправителя (preSend выполняется
                // синхронно на потоке WebSocket-контейнера, не на пуле clientInboundChannel —
                // см. javadoc класса, F8 финального ревью).
            }
